...
**Status.** Engineering-complete pending parishioner gate. Library, MCP tool wrappers (`bounty_index_finding`, `bounty_query_findings_index`), and auto-indexing in `bounty_record_finding` (best-effort, never blocks the finding write) all shipped. Each successful `bounty_record_finding` call now produces a record in `findings-index.jsonl` with the same hashed-feature vector the query path consumes. Brief-time prior injection (calling `bounty_query_findings_index` from the orchestrator's hunt-start step and threading the top-K into the hunter brief) is the next slice.
