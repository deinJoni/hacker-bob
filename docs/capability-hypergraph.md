...
**Status.** In progress — divergence-detection kernel + DI-style differential runner (`mcp/lib/doc-delta-runner.js`) shipped, plus deterministic JSONL-style persistence to `doc-delta-results.json` with a content-addressed `results_hash`. Capability pack registration (`web-doc-delta`), `bounty_http_scan` adapter for `fetch_fn`, MCP tool wrapper, triage subagent for prose-only contracts, and hunter-brief `schema_slice` integration pending.
