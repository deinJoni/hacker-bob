...
- **2026-06-07** · I9 (regression record) · OSS reachability/severity-ceiling PRODUCER dropped at the v2.1/Plane-O merge · No `reachability` emitted by `buildRepoInventory` (`mcp/lib/repo-target.js`); consumers remain wired and inert in `mcp/lib/dashboard.js`, `mcp/lib/assignment-brief.js`, and `mcp/lib/wave-brief-derivation.js`. Surfaced by the Plane-Delta verification pass; recover from blobs `d1647c0` + `1a456f1`. Restore tracked as Plane-Delta node `I9`. Engineering/fixture only; producer restore pending.
