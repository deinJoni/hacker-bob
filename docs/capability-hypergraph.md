...
**Status.** Library shipped (`mcp/lib/chain-state-tree.js` + `test/chain-state-tree.test.js`, 12 tests passing). Each node hashes `(parent_state_hash, action_canonical) → node_hash`; observed outcome hashes `(node_hash, observed_canonical) → state_hash` so a parent_state_hash uniquely identifies the world after a step. Branching falls out: two distinct actions from the same parent yield distinct node_hashes; same action with different observed yields the same node_hash but a new state_hash. `appendChainNode` upserts by node_hash, `queryChainTree` filters by `parent_state_hash`/`verdict`/`action_kind`, `frontier` returns leaf nodes (live exploration tips, default excludes `pruned`), `ancestry` walks parent_state_hash links from a leaf back to the root. Verdict enum `(pending, success, failure, pruned, branched)`. CHAIN-phase integration (the orchestrator running BFS/DFS over the frontier with replay-lease-protected backtracking) and MCP tool wrappers are pending.
