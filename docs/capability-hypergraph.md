...
**Status.** Kernel + DI-style runner shipped (`mcp/lib/auth-differential.js`, `mcp/lib/auth-differential-runner.js`). `runAuthDifferential` accepts `endpoints`, `auth_profiles`, an injected `fetch_fn`, and optional `profile_metadata`; for each endpoint it fans across every profile, computes per-profile signatures, runs the cross-profile differ, and persists to `auth-differential-results.json` with content-addressed `results_hash`. Per-profile fetch errors are recorded and do not abort the run. `bounty_http_scan` adapter (production wiring) and MCP tool wrapper (`bounty_run_auth_differential`) pending.
