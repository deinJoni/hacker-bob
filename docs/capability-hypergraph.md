...
**Failure mode it prevents.** Each evaluate starting from zero. Bob's accumulated experience invisible to itself.

**Predecessors.** Substrate: `S1, S2` (manifest provides calibration ground truth). Existing: grade verdicts, adjudication archive.

**Unlocks.** `C6` (cross-target pattern memory). Augments `C1, C2, C3, C5, C7, C8` by injecting top-K similar prior findings into evaluator briefs.

**DO.** Embedding pipeline runs on every shipped finding. Vector store (sqlite-vss or similar lightweight choice). Query API: `bounty_query_findings_index(surface_discovery_summary, top_k)` returns compact summaries. Wire into brief construction so new evaluations open with relevant priors.
...
**Status.** Engineering-complete pending parishioner gate. Divergence-detection kernel, DI-style differential runner, `bounty_http_scan` adapter, MCP tool wrapper `bounty_run_doc_delta`, assignment-brief `schema_slice` integration, and orchestrator playbook section (`prompts/roles/orchestrator.md` "Optional: Doc-vs-Behavior Differential") all shipped. The "capability pack" planned in the original hyperedge resolved to an orchestrator-driven workflow rather than a evaluator-routed pack, because C2 produces findings via differential testing the orchestrator drives, not via a per-surface evaluator sub-agent. Triage of prose-only contracts (LLM subagent) remains a quality-of-life refinement; the existing severity_class taxonomy (`security`, `info_leak_potential`, `doc_or_infra`) already drives a useful triage default. Parishioner gate stays open until C2 produces ≥1 finding accepted by a real triager.
...
**DO.** Modify EVALUATE to run every web payload across all configured auth profiles by default, recording into `I4`. Suspicious-row threshold triggers triage subagent. Web capability packs gain `multi_account_differential: true` knob.
...
**Status.** Engineering-complete pending parishioner gate. Closes once I6's brief-time prior injection ships, which it has: web evaluators get a `priors_slice` next to their `schema_slice` and other context summaries; smart-contract evaluators get the same alongside `bob_spec_status` and `rpc_pool`. Calibration label slot is reserved on every record so a future grade-verdict feedback path (which would index findings with `calibration_label: "real" | "rejected_*"`) provides ground-truth signal for similarity ranking.


**Pedagogical note.** Bob has access to its own past evaluations in principle but not in practice — there's no retrieval layer. `I6` is the retrieval layer. Each new evaluate opens with top-K similar prior findings, calibrated by which were real. This is the capability that compounds over time.

**Predecessors.** Substrate: `S1, S2`. Pillars: `I6`.

**DO.** Embedding pipeline on every shipped finding. Vector store. Query API. Wire into evaluator brief construction across all capability packs.
...
**Status.** Engineering-complete pending parishioner gate. Closes once I7's MCP tools land and the orchestrator playbook references them, both of which are now done. The orchestrator can branch attempts from the same `parent_state_hash`, prune dead branches, backtrack to a known-good `state_hash`, and rebuild chain lineage from a leaf state. Replay leases (S3) make backtracking safe: re-pinning to a prior state_hash and trying a different action_kind doesn't double-fire the destructive step the leases protect. Parishioner gate stays open until C7 produces a verified chain that flat CHAIN couldn't reach.


**Pedagogical note.** Real impact chains are 5–10 steps with dead ends. Current CHAIN handles ~2-step linear. `I7` makes CHAIN a tree search with safe backtracking. Replay leases (S3) make backtracking *safe* — without them you can't unwind a destructive step.
...
**Gate.** `C1` + `C3` each produce findings on at least one shared target. `C6` measurably reduces evaluate-to-finding wall-clock.
