...
- `--deep`: additionally inspect Kimi session log windows around flagged issues.
...
For local artifact fallback, inspect only file presence/mtimes under `~/bounty-agent-sessions/[target_domain]` plus Kimi session log files needed for `--deep`; do not dump protected raw Bob artifacts.
...
Do not dump entire transcripts. Search Kimi session log files for the target domain/session and inspect small windows around:
- phase transitions,
