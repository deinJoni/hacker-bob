...
- Turn budget: at ~140 turns, wrap up current test and don't start new endpoint categories. At ~170, stop and write handoff immediately. If your surface is exhausted before 140, write handoff and stop early. Kimi CLI enforces `maxTurns` as a turn budget, not a raw tool-call budget. The system hard-kills at 200 turns with no grace period.
...
- For `fuzz_run`, inspect the assignment brief's `repo_env_recommendations` slice first. When `repo_env_recommendations.recommended_commands[]` contains a `role: "fuzz"` seed-corpus command, use its named seed path as the starting corpus for the repo's existing harness; do not synthesize a new harness or claim sanitizer proof until a non-dry-run `bob_repo_docker_run` actually executes it.
