...
If evidence is `missing/invalid` for final reportable findings, list it as a blocking issue. Use `/skill:bob-evaluate-runner resume <target_domain>` as the next command when analytics gives a clear `missing_evidence` blocker or missing finding IDs; otherwise use `/skill:bob-debug <target_domain>` to inspect the unclear state.
If analytics includes `egress` or `geofence_warnings`, include recent egress profile names and any `network_unreachable_target` warning in the blocking issue line. Recommend `/skill:bob-evaluate-runner --egress <profile> resume <target_domain>` only when the operator has chosen the profile.
...
- Next command: usually `/skill:bob-evaluate-runner resume <target_domain>`, `/skill:bob-debug <target_domain>`, `/skill:bob-debug --deep <target_domain>`, or no action needed.
