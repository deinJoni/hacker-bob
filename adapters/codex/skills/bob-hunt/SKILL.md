...
- Header fields: Domain: [domain]; Wave: w[wave]; Agent: a[agent]; Surface: [surface_id]; Capability pack: [assignment.capability_pack]; Brief profile: [assignment.brief_profile]; Hunter agent: [assignment.hunter_agent]; Context budget: [assignment.context_budget]; Handoff token: [only this agent's handoff_token from bounty_start_wave.data.assignments]; Checkpoint mode: [normal|paranoid|yolo].
- First action inside the worker: call bounty_read_hunter_brief({ target_domain: '[domain]', wave: 'w[wave]', agent: 'a[agent]' }) and use .data.run_context.context_budget plus .data.technique_packs.selected when present.
...
- Call `bounty_log_technique_attempt` when you select, reject, attempt, validate, fail, or abandon a technique pack. Use MCP tools only; never write `technique-attempts.jsonl` or `technique-pack-reads.jsonl` through Bash.
