...
- For `fuzz_run`, read `repo-env.json` first. When `recommended_commands[]` contains a `role: "fuzz"` seed-corpus command, use its named seed path as the starting corpus for the repo's existing harness; do not synthesize a new harness or claim sanitizer proof until a non-dry-run `bob_repo_docker_run` actually executes it.
