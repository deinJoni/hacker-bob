...
- Evaluator completion correctness is MCP-owned through `bob_finalize_agent_run`; Codex has no Bob stop hook; MCP finalization is the correctness boundary.
...
3. For each REPORTABLE finding (filtered by the gate above), branch first by `finding.capability_pack`, then by `finding.surface_type`:
   ...
   **OSS repo findings** (`capability_pack` starts with `"oss_"`):
   ...
   - CVSS v3.1 (informational): the MCP derives the CVSS v3.1 base vector and score server-side from the finding's structured `cvss_inputs` and renders them in report.md as an INFORMATIONAL annotation. Supply `cvss_inputs` (the CVSS v3.1 base enums: `attack_vector`, `attack_complexity`, `privileges_required`, `user_interaction`, `scope`, `confidentiality`, `integrity`, `availability`) on the candidate claim for reportable findings; do NOT hand-author a vector string. For OSS findings, derive AV from the reachability prose (`network` -> attack_vector network, `local` -> attack_vector local) and set PR/UI from maintainer/user-action prerequisites; when `cvss_inputs` is omitted but a reachability assertion is present, the MCP carries that classification into `attack_vector` automatically. The grade verdict severity stays authoritative — the rendered CVSS is informational only and never implies a severity divergence from the grade verdict.
