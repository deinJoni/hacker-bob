...
- Confirm primary file/path evidence with `bounty_repo_check({ target_domain, file_path, pattern?, check_type? })`.
...
- Use `bounty_repo_docker_run({ target_domain, command, dry_run: true })` before proposing a Docker repro. Run it for real when the prepared image exists, `$ARGUMENTS` included `--build`, or the operator otherwise approved the build/replay path. Never silently replace dynamic proof with prose.
...
3. Run CHAIN, VERIFY, optional EVIDENCE, GRADE, and REPORT using the same MCP-owned gates as `$bob-hunt`.
...
4. **OSS repo (`replay_tool: "bounty_repo_check"`)**: parse the finding for a repo-relative file path, manifest, or config path; call `bounty_repo_check({ target_domain, file_path, pattern?, check_type: "verification_replay" })`. If the finding includes a concrete build/test reproducer and `repo-env.json` has a prepared image, use `bounty_repo_docker_run({ target_domain, command, timeout_ms? })` for bounded replay. Keep only findings whose file-level evidence still exists and whose impact is tied to reachable project behavior, dependency metadata, CI config, or documented security behavior.
...
3. **Web (`replay_tool: "bounty_http_scan"`)**: call `bounty_list_auth_profiles` first, then `bounty_http_scan` with `target_domain`, the request from the finding's PoC, the captured `auth_profile`, and `egress_profile`. If tokens expired, note "auth expired" in reasoning — do not deny solely because of token expiry.
4. **OSS repo (`replay_tool: "bounty_repo_check"`)**: parse the finding for a repo-relative file path, manifest, or config path; call `bounty_repo_check({ target_domain, file_path, pattern?, check_type: "final_verification" })`. If the finding includes a concrete build/test reproducer and `repo-env.json` has a prepared image, use `bounty_repo_docker_run({ target_domain, command, timeout_ms? })` for bounded replay. Confirm only when the file-level evidence is still present and the reasoning can point to the repo artifact that supports the claim.
