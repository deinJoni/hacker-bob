...
3. Call `bounty_repo_inventory({ target_domain })`. This writes `repo-inventory.json` and a compatible `attack_surface.json`. The result includes a `reachability` block (`max_credible_severity_ceiling`, `network_reachable`, per-surface `attack_vector`/`severity_ceiling`) and `residual_hunt_targets` (recently-patched security fixes). **Target-acceptance check (warn, never block):** read `reachability.max_credible_severity_ceiling` and `reachability.network_reachable`. If the max ceiling is `medium` or `low` and no surface is network-reachable, emit an explicit operator warning before continuing — e.g. "⚠️ This target looks structurally capped around MEDIUM: no unauthenticated network-reachable, RCE, or write-primitive surface was detected (it reads like a local-file parser/library, AV:L). Highest-ceiling surface: `<id>`. Proceeding — but CRITICAL/HIGH is unlikely here unless an incomplete-fix residual or a network entrypoint is found. Consider narrowing to the highest-ceiling surface or choosing an AV:N target (network daemon/server, supply-chain pipeline)." Always continue after warning; this check informs the operator, it does not stop the run.
...
- Severity-ceiling steering: each surface carries `severity_ceiling`, `attack_vector`, and `network_reachable`. On `attack_vector: "network"` / `severity_ceiling: "critical"` surfaces, do NOT stop at an out-of-bounds READ (HIGH ceiling) — pursue the write/UAF/RCE primitive that reaches CRITICAL and document the unauthenticated network reachability path (which listener/daemon feeds the parser). On `attack_vector: "local"` surfaces an honest MEDIUM is the realistic ceiling; state that rather than inflating. The ceiling is the surface's best case, not a per-file verdict: a native surface may also carry `network_reachable_anchors[]` / `network_reachable_dirs[]` (the AV:N listener paths) and `local_only_candidate_dirs[]` (AV:L parser paths) — steer hunters to the network-reachable paths for CRITICAL and accept honest MEDIUMs for findings in local-only dirs.
- Incomplete-fix residual hunting (highest-yield HIGH method): the native surface carries `residual_hunt_targets[]` — recently-patched security fixes from git history and the changelog. For each, read the patch, then hunt the SIBLING code the fix did NOT cover (same struct, the adjacent count/length field, a parallel branch, the unbounded loop that mirrors the one just bounded). The residual of a recent CVE/GHSA patch is the most reliable HIGH on an otherwise-hardened codebase.
